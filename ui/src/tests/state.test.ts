...
describe('TEST-UI: Global State Runes', () => {
    ...
    it('should initialize instancesMap with exchange-symbol key', () => {
        ...
        expect(app.instancesMap['BTC-USDT'].terms[1].priceText).toBe('--');
    });
    ...
    it('should route snapshot data by exchange key to correct pair', () => {
        ...
        app.instancesMap['BTC-USDT'].terms[1].priceText = '50000.00';
        app.instancesMap['BTC-USDT'].terms[1].latestSnapshot = { mid_price: '50000.00', exchange: 'Hyperliquid', symbol: 'BTC' };

        expect(app.instancesMap['BTC-USDT'].terms[1].priceText).toBe('50000.00');
