...
describe('TEST-UI: State Reactive Effects', () => {
    ...
    it('should register all four timeframes on pair init', () => {
        ...
        expect(pair.terms[1]).toBeDefined();
        expect(pair.terms[5]).toBeDefined();
        expect(pair.terms[30]).toBeDefined();
        expect(pair.terms[900]).toBeDefined();

        // Micro-term defaults
        expect(pair.terms[1].priceText).toBe('--');
        expect(pair.terms[1].indicators).toEqual({});
    ...
    it('should update chart telemetry via snapshot assignment', () => {
        ...
        expect(pair.terms[1].latestSnapshot).not.toBeNull();
        const snap = pair.terms[1].latestSnapshot!;
    ...
    it('should snapshot per-timeframe state independently', () => {
        ...
        app.instancesMap['BTC-USDT'].terms[1].priceText = '65000.00';
        app.instancesMap['BTC-USDT'].terms[1].indicators = {
            rsi: { raw_value: 62.5, normalized: 0.44, state_label: 'BULLISH_MOMENTUM' },
        };

        // Set values on ETH small-term
        app.instancesMap['ETH-USDT'].terms[5].priceText = '3200.00';
        app.instancesMap['ETH-USDT'].terms[5].indicators = {
            rsi: { raw_value: 45.0, normalized: -0.18, state_label: 'BEARISH_MOMENTUM' },
        };

        // BTC micro-term unchanged
        expect(app.instancesMap['BTC-USDT'].terms[1].priceText).toBe('65000.00');
        // ETH small-term holds its value
        expect(app.instancesMap['ETH-USDT'].terms[5].priceText).toBe('3200.00');
        // ETH micro-term still default
        expect(app.instancesMap['ETH-USDT'].terms[1].priceText).toBe('--');
    });
});
