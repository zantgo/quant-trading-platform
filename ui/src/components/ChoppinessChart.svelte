<script lang="ts">
    ...
    const chopCoalescer = makeChartCoalescer(app, () => pairKey, () => slot, (snap, tfPlain) => {
        const timeSec = snap.timestamp as number;
        if (timeSec < _lastHistoryTime) return;
        const v = iRaw((tfPlain.indicators ?? {}) as IndicatorMap, 'choppiness');
