<script lang="ts">
    ...
    onMount(() => {
        ...
        spreadSeries = chart.addSeries(LineSeries, { color: '#f48fb1', lineWidth: 1, priceLineVisible: false });
        spreadSeries.createPriceLine({ price: 0.1, color: '#4c525e', lineWidth: 1, lineStyle: LineStyle.Dashed, axisLabelVisible: true, title: 'NORMAL' });
        spreadSeries.createPriceLine({ price: 0.3, color: '#ffab40', lineWidth: 1, lineStyle: LineStyle.Dashed, axisLabelVisible: true, title: 'WIDE' });
        spreadSeries.createPriceLine({ price: 1.0, color: '#ef5350', lineWidth: 1, lineStyle: LineStyle.Dashed, axisLabelVisible: true, title: 'EXTREME' });
        ...
        if (onScreenshotReady) {
            onScreenshotReady(() => {
                ...
                link.download = `${pairKey}_${timeframe}s_spread.png`;
                link.href = dataUrl;
                link.click();
            });
        }
    ...
    $effect(() => {
        ...
        fetchIndicatorHistoryOnce(pairKey, timeframe, slot).then((h: IndicatorFlatHistory | null) => {
            ...
            const pts = pairsFromHistory(h, 'spread');
            if (pts.length > 0) {
    ...
    $effect(spreadCoalescer.effect);
    onDestroy(spreadCoalescer.destroy);
...
<div class="chart-container" bind:this={container}>
    ...
    {#if liveReceived && !showEmptyOverlay}
        <div class="live-pill" title="Live feed active (Layer 2: data publishing, no signal emitted yet)">
            ⚡ LIVE
        </div>
    {/if}
</div>
...
<style>
    ...
    /* Layer 4: subtle ⚡ LIVE pill that confirms the WS feed is connected
       and pushing fresh values, but no discrete signal has fired on the
       current bar (the indicator is data-only for spread). The trader
       reads this as "feed is healthy, just no signal right now" — distinct
       from the empty-overlay which means "no data ever arrived". */
    .live-pill {
        position: absolute;
        top: 6px;
        left: 8px;
