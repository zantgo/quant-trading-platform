<script lang="ts">
    ...
    let hullSeries: ISeriesApi<'Line'>;
    ...
    onMount(() => {
        ...
        hullSeries = chart.addSeries(LineSeries, { color: '#ff8a65', lineWidth: 2, priceLineVisible: false });
        ...
        if (onScreenshotReady) {
            onScreenshotReady(() => {
                ...
                link.download = `${pairKey}_${timeframe}s_hull_ma.png`;
                link.href = dataUrl;
                link.click();
            });
        }
    ...
    $effect(() => {
        ...
        fetchIndicatorHistoryOnce(pairKey, timeframe, slot).then((h: IndicatorFlatHistory | null) => {
            ...
            const pts = pairsFromHistory(h, 'hull_ma');
            if (pts.length > 0) {
                hullSeries.setData(pts);
    ...
    $effect(hullCoalescer.effect);
    onDestroy(hullCoalescer.destroy);
