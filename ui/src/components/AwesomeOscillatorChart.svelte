<script lang="ts">
    ...
    onMount(() => {
        ...
        aoSeries = chart.addSeries(HistogramSeries, { base: 0, priceLineVisible: false });
        aoSeries.createPriceLine({ price: 0, color: '#4c525e', lineWidth: 1, lineStyle: LineStyle.Solid });
        ...
        if (onScreenshotReady) {
            onScreenshotReady(() => {
                ...
                link.download = `${pairKey}_${timeframe}s_awesome.png`;
                link.href = dataUrl;
                link.click();
            });
        }
    ...
    $effect(aoCoalescer.effect);
    onDestroy(aoCoalescer.destroy);
