...
">
"> **Implementation status (v11.9 — duration-keyed timeframes).** A timeframe IS its duration in seconds; the named-slot world is erased from code, docs, and tests. The closed pool is the 14-duration set (1 s, 3 s, 5 s, 15 s, 30 s, 1 m, 3 m, 5 m, 15 m, 30 m, 1 h, 4 h, 12 h, 1 d) in `core_domain::SUPPORTED_DURATIONS` with derived labels (`duration_label` / `duration_label_upper` / `duration_from_label`). The ACTIVE set is `[workspace].timeframes: Vec<u64>` (any subset, 1..=14 entries, ascending, default the fastest eight `[1,3,5,15,30,60,180,300]`); legacy named TOML keys are **hard-rejected at load** with a migration message (no fallback). Inactive durations are inert (no pipeline, no snapshot, no WS socket, no bootstrap). Live-editable: `POST /api/config` accepts `timeframes` (validated 1..=14 unique pool members) and recharges running instances; `GET /api/instances` carries `active_secs`. Per-instance per-duration overrides: `[workspace.instances.timeframes.<secs>]` (complete `TimeframeConfig` per duration, applied over the `duration_profile` row). Strategy mapping follows the ACTIVE extremes (`ladder_roles` defaults `1d`/`1s`, decision/stop fall back to the slowest ACTIVE duration). BTE bound runs replay the ACTIVE durations ∩ ≥ 60 s — empty for sub-minute-only ladders (`400 no_active_ladder`); standalone default ladder `[60,180,300,900,1800,3600,14400,43200,86400]`. Wire: `MarketSnapshot.timeframe_secs` + derived `timeframe_label`; `/ws` accepts `?tf=<secs>` or `?slot=<label>`. UI: secs-keyed `InstanceState.terms: Record<number, …>`, `activeDurations`, 14 duration toggles (default 8, min 1), rails/grids/exports/badge keys duration-keyed. Navigation: boot / launch / recovery always land on the Market Monitor **Overview**; Data Infrastructure defaults to **Overview**; the standalone Settings page is erased — MME **Settings** shows General settings (no instance) or Workspace settings (instance selected). See `docs/conceptual-foundations/01-04-timeframe-model.md` §2. All suites + `test-doc` green. **v11.12.3 round:** the MME layer headers are a FIXED two-row layout (row 1 identity+live badge+ghost trail; row 2 context pills + LIVE/pair/EXPORT right) stable at any width; the Alignment header loses its TFs chip; the Analysis chip is renamed **State Confidence** and confidence VALUES render white; every MTF table (Indicators/Signals/Divergences/Levels) scrolls BOTH axes inside its own container with the TF header row sticky INSIDE it; the Settings internal navbar is **General | Timeframes | Instance** (renamed from Workspace to avoid the app-navbar clash) with SAVE immediately left of EXPORT and a save state that can never stick in "saving" (clean sections return success; the ladder saves with no instance; the backend recharges instances concurrently); the docs corpus is re-stamped to v11.12 and corrected to the observe-only build. **v11.12.4 round:** the MTF scroll ports declared the INVALID CSS `overflow: auto both` (dropped by the browser → tables painted over the sections below and spilled outside their containers); they now declare valid two-axis `overflow: auto`, and EVERY MTF table (per-GROUP indicator tables included — the shared floating TF header is gone) scrolls vertically + horizontally inside its own container with a sticky in-container TF header; the Signals table’s first column reads SIGNAL KIND; the header chip is `Timeframes: N/14` (grey label, white value, active-ladder count) on Metrics-MTF and Alignment and is erased from Opportunities; the unified Settings action row carries the active tab’s section title at the LEFT corner in engine-header style (General Settings / Timeframes and Indicators Settings / Instance Settings) with the in-card Timeframes title removed. **v11.12.5 round:** the Market Overview header follows the same row order as every MME layer header — title row → L7 badge + ghost trail → scan/meta pills (badges FIRST, pills under them); the L7 badge renders at 14 px (one step above the per-tab badges). **v11.12.6 round:** every page ends with the SAME 32 px bottom breathing room (Charts/Metrics 14→32, Overview/Settings/DIE dashboards 24→32, MME panels normalized with `.panel > :last-child { margin-bottom: 0 }`), and each MTF indicator GROUP table header now labels its two previously-blank columns — `INDICATOR` (left) and `AGREEMENT` (right, spanning the badge + average columns). **v11.12.7 round:** the settings internal navbar is a byte-level clone of the app sub-tab row (`--bg-elev-2` band, NO full-width divider, the glowing `inset 0 -2px 0 0 var(--text)` underline under the selected tab) and the settings action header sits on pure black with no separator line; the Metrics bottom gap is REAL (`.facetBody` clips; `MtfView .view` flex-scrolls internally so the 32px band is never painted over) and Charts gets 32px at the end of `.timescale-charts` scroll; the MTF indicator trailing titles split into `AGREEMENT` (badge column) + `AVG` (value column). **v11.12.8 round:** Charts and Metrics share ONE `TimeframesRail` component (extracted from TerminalMonitor) — identical 140 px rail, white `TIMEFRAMES` title at 11 px, lowercase `1s`…`1d` labels, and a white selected state (2 px white left bar + inset glow, `rgba(255,255,255,.07)` background, white label) replacing the old cyan accent, which is gone from both rails; the Charts page becomes a row (rail flush under the navbar) with the `ChartToggles` header moved to the right of the rail and restyled to the canonical page-header band (`rgba(10,10,10,.5)`, `10px 16px` padding, hairline) plus 10 px/50% group labels; functionality untouched (same handlers, scrolling, exports). **v11.12.9 round:** the Charts chart card ends at content height with the 32px page band OUTSIDE it (`.single-column` owns it; removed from `.timescale-charts`); the Metrics `LayerHeader` is full-bleed — flush against the TIMEFRAMES rail (`.headerBleed { margin: 0 -14px }`), same geometry as the Charts toggles; MTF tables no longer collapse when every section is expanded (`.section`/`.tblSection`/`.headingRow` are `flex-shrink: 0`, so `MtfView .view` scrolls internally) and single-TF facets scroll the page again via `.facetBodyFlow` (the capped/clipped `.facetBody` is MTF-only); titles unified — MME layer identity + MARKET OVERVIEW + INSTANCES adopt the white 10px mono charts-label style, while the Charts group labels, DIE dashboard titles, Settings section titles and Home panel titles (shared `engine-dashboard .title`) turn white colour-only, and the ASSET RANKINGS title copies the INSTANCE STATUS rule (10px/.1em/#64748b). **v11.12.10 round:** the MARKET OVERVIEW + MME layer-identity title words grow to 12 px (white/mono style kept); the DIE headers lose their duplicated titles — `DashboardHeader.tabLabel` is optional and DIE no longer passes it (the title was printed left + right on every DIE tab), and `DIEConnectionSettings`' band drops its duplicate "Connection Settings" title (its right label reads `Settings`, matching TAE/PME/PAE/BTE), leaving one title per page. **v11.12.11 round:** the Workspace tab's INSTANCES title matches the Settings section titles exactly (13 px sans, 700, .08em, white — `engine-dashboard .title`), replacing the 10 px mono page-header style. **v11.12.12 round:** the MME sub-tab labels are singular — OPPORTUNITY / RISK (matching their layer headers); every MTF column title now shares its column's alignment — the indicator grid's AGREEMENT/AVG titles and the stacked tables' TOTAL title are centered over centered values (AVG numbers and TOTAL badge groups centered; the agreement badge was already full-width centered), with the TF columns already center/center. **v11.12.13 round:** every **MTF column title** shares ONE style (11 px / 700 / uppercase / white) and the duplicated duration suffix line (`1s` under `1S`) is gone; **Structural Anchors** gets a 12 px white section title over white 10 px tile titles and its neutral cyan (`vEmph`/`vMono`/distance/ladder meters) is swept to white/grey; the **Metrics facet tabs** mirror the TIMEFRAMES rail (selected = white label + white underline + `rgba(255,255,255,.07)` bg; counts grey→white, no cyan); `ASSET RANKINGS` and `INSTANCE STATUS` titles share an explicit sans family for guaranteed parity; the **Overview scan row** loses the `2/2 pairs` pill and unifies `Last scan` / `Auto-refresh` / `Instances` / `Sys risk` / `Sync` (grey sentence-case labels, always-white ALL-CAPS values, no state colours); **Inter weights 500–800 (+400 italic) are now actually loaded** — previously only the 400 face shipped with `font-synthesis: none`, so every bold declaration rendered regular (the root cause of the “not bold” settings/`INSTANCES` titles); the Watchlist modal copy drops “(default 5 min)”; the Timeframes card header replaces the `config [workspace].timeframes`/`LIVE` chip + instruction line with a clear `TIMEFRAME | INDICATOR` title; and the signal meta tags spell out `STRENGTH` / `CONFIDENCE` / `AGE` (all caps, grey key + white value, same size) across the Signals/Levels/Indicators facets, the Liquidity panel and the MTF tooltips. **v11.12.14 round:** the Inter 500–800/italic import is REVERTED (the global weights had turned the whole app bold); ONLY the Settings tab titles (`.sectionTitle` on the UnifiedSettings action-row h2) and the Workspace `INSTANCES` title re-enable synthesis (`font-synthesis: weight`) so they alone read bold; the Timeframes card header splits into true two-pane columns — TIMEFRAME over the duration rail and `| INDICATOR` over the parameters pane, with `.tf-shell` now a grid so the header tracks the real pane geometry. **v11.12.15 round:** the Instance Status table renders the dim pair-key only when it differs from the instance symbol (live instances no longer print `ETH-USDC ETH-USDC`); the shared TIMEFRAMES rail is SINGLE-LINE — the duplicated secs line is gone from Charts and Metrics and the MTF entry reads `Multi-TF`. **v11.12.16 round:** the Instance Status row's bold label is ALWAYS the base symbol derived from the canonical pair key (dim full pair beside it) — rows render `BTC · BTC-USDC` whether `inst.symbol` holds the bare base or the full pair, never a lone `BTC-USDT`. **v11.12.17 round:** crash recovery runs the SAME preparing gate as a fresh launch — after “Recover last session” the welcome screen shows one row per recovered instance (`waiting for first snapshot… → ready ✓`, 60 s cap → CONTINUE) and lands on the Overview only once the first snapshots arrive (previously it landed immediately and showed an empty Overview);  the Resume/Quit card is suppressed mid-recovery and a zero-instance recovery still lands immediately. **v11.12.19 round:** the venue REST probes and the clock monitor no longer misfire on a slow network — `symbol_exists` (Bitget + Hyperliquid) retries transport errors (3 attempts, 30 s per attempt + 10 s connect, 1 s/2 s backoff, classified root-cause errors via `network-adapters::adapters::http`) instead of failing the single 10 s request, and the hard block until the venue answers stays as-is; the NTP monitor gained `[clock_monitor].max_rtt_micros` (default 1 s) — samples slower than that are `DriftVerdict::Unreliable` (logged, never a breach, `breach_action=panic` cannot fire) because the offset uncertainty is ≈ rtt/2 and a multi-second round-trip cannot prove a drift. **v11.12.20 round:** the overlay toggles read/write the FASTEST ACTIVE duration (`terms::overlayRepTerm` — `terms[1]` only when 1s runs) across `ChartToggles`, `chartOverlays` and the Workspace overlay-chips baseline; the hardcoded 1s slot had frozen every TF pill on ladders that deactivated 1s (the light never flipped because `syncAll` only writes active slots). The MEDIUM EMA pill lights its line's colour (`#ef5350`). Above-1-minute history reliability: both historical fetchers keep the pages already fetched on timeout (partial warm) and retry a failed page (2×, 1 s/2 s backoff); the `/api/history` DB fallback carries the persisted EMA-stack/Bollinger/VWAP series via a new `query_recent_overlay_rows` query, guarded to ≥60 s slots (sub-minute stays live-only), and a visible `NO HISTORICAL DATA` note renders when a ≥1 min slot still has no overlay history (live data clears it). No changes to sub-minute behavior or the default (1 s-active) ladder. **v11.12.21 round:** the crash-recovery gate is a HARD gate — the "Recover last session" preparing screen never auto-lands and never auto-passes: CONTINUE stays blocked until every recovered instance has its first snapshot, non-successful rows render per-instance states (`waiting for first snapshot…` → amber `still warming — remove to continue` after the 60 s mark, polling continues so a late snapshot still clears them) with a ✕ ELIMINATE action (DELETE `/api/instances/:id` + store removal; a failed DELETE re-marks the row `removal failed — still running` and keeps the gate blocked), and landing always requires the operator's CONTINUE click (all rows ready or eliminated — zero rows continue to an empty workspace). The fresh-launch preparing step keeps its existing auto-land/timeout behavior; the launch gate is untouched. **v11.12.22 round:** the recovery flow is the INSTANCES step itself (supersedes the v11.12.21 separate preparing gate): "Recover last session" flips the wizard to step 3 with one chip per recovered instance (pairs read from `GET /api/config.instances` Running entries, so they render immediately even while the background boot spawn is still retrying its venue symbol check — on slow networks that retry can run for minutes and `/api/instances` stays empty), the ADD area and BACK are hidden, REVIEW/LAUNCH are unreachable, and the wizard footer can never appear while `recovering` (that leak let CONTINUE walk MODE→…→LAUNCH mid-recovery and land on an EMPTY workspace — the reported bug). Chips flip `waiting for first snapshot…` → `ready ✓` (the poll seeds the store for pairs once `/api/instances` lists them, so the WS attaches); after 60 s they turn amber `still warming — remove to continue` and STILL block CONTINUE. CONTINUE lands directly when every chip is ready or eliminated. The ✕ eliminates: a spawned instance goes through `DELETE /api/instances/:id` (failure re-inserts the chip as `✕ unavailable`, keeps the store entry and the gate blocked); a still-spawning one drops immediately (cancel-guard) and a watcher `DELETE /api/instances/by-pair/:pair` the moment it materializes (10-min cap, console warning). Fresh-launch `stagedReady`, the preparing warm gate and steps 1/2/4 are untouched.
 **v11.12.18 round:** duplicate timeframe labels are erased app-wide — the chart column header renders the single duration label (`1M`, was `1M (1M)`), and the Instance Status / Timeframe Status rows render `1S` alone (the `· 1s` sub-label is gone from `LiveTerminal`, `InstanceStatusTable` and `TfStatusTable`; both dead `.tf-duration` CSS rules removed); pair display is hyphenated app-wide — the Workspace list, right Instances panel, delete errors/confirm modal, app-header chip, launch wizard and Metrics header render the canonical `BTC-USDT` (slash-form search queries still match; `pairDisplayFor` delegates to `pairKeyFor`); the Settings Timeframes editor drops the decorative `|` (`TIMEFRAME` / `INDICATOR`), the rail's `· <secs>` suffix and the pane sub-title's `· <secs>` repetition (`3m — INDICATOR PARAMETERS`), and the action-row titles are renamed **General Configuration / Timeframes and Indicators Management / Instance Configuration**. All suites + `test-doc` green.
>
