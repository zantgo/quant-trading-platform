...
The **Trading Platform** processes high-resolution exchange telemetry and transforms raw data into real-time technical indicator visualizations. It computes 52 technical indicators across 8 functional groups (Trend, Momentum, Volume, Volatility, Structure, Regime, Institutional, Derivatives Data) with 101 signal-kind declarations across 12 SignalKind types (Divergence, Crossover, Threshold, Breakout, BandTouch, ZeroLineCross, CompressionRelease, LevelTest, TrendFlip, VolumeClimax, StackChange, PatternForming), 8 of which support bull/bear divergence detection. All computation runs in Rust, streaming live data to a Svelte 5 dashboard via WebSocket. **The current UI build is observe-only** (market monitoring); paper/live execution remains a backend/CLI/API capability.

> **Implementation status (v11.12).** All six engines are **implemented**: DIE (ingestion/reconstruction/persistence); MME (52 indicators across the ACTIVE duration set, decision layer, liquidity intelligence); TAE as the setup executor on the unified execution engine (Observe is the default posture and the only mode offered by this UI build; paper/live backends — `PaperSimulation`, Hyperliquid `LiveBroker`, `BitgetLiveBroker` — remain available via config/CLI/API); PME as the informational portfolio mirror; PAE with live analytics plus the recorded-decision backtest and the full significance treatment; BTE with deep-history and recorded-decision replay persisted to data-science tables. Observe sessions never dispatch orders. See [`docs/ROADMAP.md`](docs/ROADMAP.md).
