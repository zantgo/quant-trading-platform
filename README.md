...
The **Trading Platform** processes high-resolution exchange telemetry and transforms raw data into real-time technical indicator visualizations. It computes 52 technical indicators across 8 functional groups (Trend, Momentum, Volume, Volatility, Structure, Regime, Institutional, Derivatives Data) with 101 signal-kind declarations across 12 SignalKind types (Divergence, Crossover, Threshold, Breakout, BandTouch, ZeroLineCross, CompressionRelease, LevelTest, TrendFlip, VolumeClimax, StackChange, PatternForming), 8 of which support bull/bear divergence detection. All computation runs in Rust, streaming live data to a Svelte 5 dashboard via WebSocket. **The current UI build is observe-only** (market monitoring); paper/live execution remains a backend/CLI/API capability. The platform is designed for traders and market analysis, providing a high-performance interface for monitoring market conditions, identifying technical signals, and interpreting real-time market behavior.
...
The platform is a 10-crate Cargo workspace + Svelte 5 frontend. The six logical engines (DIE, MME, TAE, PME, PAE, BTE) are mapped onto these physical crates:
...
- `crates/market-analyzer` — 52 technical indicators, signals, multi-TF pipeline, decision support, liquidity math (MME).
...
- `crates/backtesting-engine` — Candle archive + backfill, deep-history MME-pipeline replay, recorded-decision replay, DS persistence (BTE).
...
The complete institutional documentation set lives under [`docs/`](docs/), organized into conceptual foundations, matrix data contracts, per-engine specifications, integration/API references, UI/UX layouts, and operations & compliance guides.

| Document | Audience | Description |
|---|---|---|
| **[Global Architecture](docs/conceptual-foundations/01-02-global-architecture.md)** | Developers | Two-dimensional framework: 6 engines x sequenced analytical layers |
...
| **[Timeframe Model](docs/conceptual-foundations/01-04-timeframe-model.md)** | Developers | Duration-keyed timeframe model: 14-duration pool (`1s`…`1d`) with a live-editable ACTIVE set |
| **[Matrices](docs/matrices/02-00-matrix-field-ownership.md)** | Developers | Physical schemas and JSON contracts for the matrix family (MME + DIE + BTE) |
| **[Data Infrastructure Engine](docs/engines/data-infrastructure-engine/03-01-01-die-overview-spec.md)** | Developers | Ingestion, reconstruction, persistence, and data-quality layer specs |
...
| **[Trade Automation Engine](docs/engines/trade-automation-engine/03-03-01-tae-overview-spec.md)** | Developers | Setup executor + unified execution engine specs (Observe default; paper/live backends) |
...
| **[Backtesting Engine](docs/engines/backtesting-engine/08-01-bte-overview.md)** | Developers | Candle archive + backfill, historical runner, recorded replay, DS persistence |
...
| **[Operations & Compliance](docs/operations-and-compliance/08-01-user-manual.md)** | Operators | User manual, pre-trade risk controls, connection resilience/quality, clock monitor |
