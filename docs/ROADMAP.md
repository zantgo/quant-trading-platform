...
| **MME — Market Monitoring** | `crates/market-analyzer` (52 indicators, duration-keyed TF pipeline, signals, multi-TF synthesis, MarketContext, Decision Matrix) | `LiveTerminal`, `TerminalMonitor`, `AlignmentPanel`, `OpportunitiesPanel`, `RiskPanel`, `AnalysisPanel`, `RecommendationPanel`, `LiquidityPanel`, `StructuralAnchorsStrip` (all WS-fed) | **Yes — implemented** | Implemented |
...
- `crates/portfolio-supervisor/src/setup_executor.rs` — `extract_top_setup` (per-duration aggregation across the ACTIVE durations' latest completed snapshots; Actionable/READY/min-RR filter, zone-midpoint geometry), per-symbol state machine (Idle → PendingEntry → PositionOpen), LEVEL/SIGNAL/REPLACED invalidation, direction-flip market close, safety + lifecycle gates, global position cap, setup-fingerprint dedup, `compute_risk` sizing + projected risk/return.
...
- `crates/execution-daemon/src/main.rs` — 1s setup-executor loop per instance (reads all ACTIVE duration buffers — `[workspace].timeframes`, v11.9; fills → executor tick → equity sync), 8h funding, STOPPING flatten, boot recovery.
...
| C5. PME / TAE communication contracts under load | `portfolio-supervisor` | Stress test demonstrates veto-loop responsiveness under 10 symbols × 14 durations |
