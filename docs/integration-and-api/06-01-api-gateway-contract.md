...
|--------|------|---------|----------|
| `GET` | `/api/session/status` | — | `{ active: bool, currency: string, exchange: string, instance_count: u32, mode?: "observe"\|"paper"\|"live", capital?: number, session_id: number, interrupted: bool, interrupted_session?: {…} }` — **v11.12: `active` is the OPERATOR-INTENT flag** (`SessionState::ui_active`), set only by `POST /api/session/init` and `POST /api/session/recover` and cleared by quit/discard — a fresh boot always reports `active:false` even while the background instance respawn runs, so the Welcome gate is mandatory. `session_id` (v10) + `interrupted`/`interrupted_session` (v11.6) feed the session chip and the Recover/Discard card. |
| `POST` | `/api/session/init` | `{ exchange: "Hyperliquid"\|"Bitget", currency: "USDT"\|"USDC", mode?: "observe"\|"paper"\|"live", portfolio_capital_usd?: number }` — `mode` + `portfolio_capital_usd` are the session defaults for instances created during the session. `observe` = monitoring only (no orders), `live` requires an active API key for the chosen exchange (otherwise `400` with a clear message). | `{ success: bool, message: string, mode?: string, portfolio_capital_usd?: number }` | — v11.7: the UI exposes Observe only; the endpoint keeps accepting `observe|paper|live`.
| `POST` | `/api/session/quit` | — | `200 OK` + JSON (cleanup result) → cleans all instances and clears `ui_active` (corrected 2026-08-17 — the handler returns `200` with a body, not `204`) |
| `POST` | `/api/session/recover` | — | **Served (v11.6).** Recover the interrupted session detected at boot: activates it with the posture derived from the persisted instances (empty workspace ⇒ observe), respawned instances paper/live boot instance PAUSED, sets `ui_active = true`, marks the session row recovered. |
| `POST` | `/api/session/discard` | — | **Served (v11.6).** Discard the interrupted session: instances + settings wiped to defaults, the row marked discarded, `ui_active` stays false (back to the Welcome wizard). |
...
| Method | Path | Request | Response |
|--------|------|---------|----------|
| `GET` | `/api/config` | — | `{ api_key_configured: bool, symbols: string[], candles, indicators, instances, indicator_registry, api_failover, timeframes }` — the full `AppConfig` surface including the `[api_failover]` block (see §2.2.1 below). v7.2 added `slow_timeframe` / `macro_timeframe` (the then-configurable ladder durations); since v11.1 the ladder was fixed; those legacy fields were echoed for backward compatibility during v11.x. **v11.9 replaces them with `timeframes: [u64]`** — the ACTIVE duration set, any 1..=14 unique members of the 14-duration pool, default the fastest eight (`[1, 3, 5, 15, 30, 60, 180, 300]`); legacy named keys are hard-rejected at load. See [01-04 §2](../conceptual-foundations/01-04-timeframe-model.md). v7.3 adds the engine-settings blocks the TAE/PME/PAE **Settings** tabs render: `liquidity`, `minimal_tae`, `analytics`, `risk_limits`, `safety`, `fees`, `leverage`, `execution`, `scoring`. v7.4 adds `activation` (workspace indicator/signal activation defaults) — and the settings blocks are **editable** via `POST /api/config` (see below), no longer read-only. |
| `POST` | `/api/config` | The `GET /api/config` response body (partial update) | `200 OK` on success. The operator-editable groups (`candles`, `indicators`, `instances`, `api_failover`, `timeframes` (v11.9 — validated 1..=14 unique pool members, `400` otherwise), and since v7.4 `minimal_tae`, `safety`, `risk_limits`, `analytics`, `scoring`, `execution`, `fees`, `leverage`, `activation`) are merged into the currently loaded workspace config and persisted to `config.toml` (platform sections preserved via `save_workspace`); `config_version` increments on every accepted save. The body does **not** require `id`/`name` — the read-only fields echoed by GET (`api_key_configured`, `symbols`, `indicator_registry`) are accepted and ignored. v7.4: M8-style range validation rejects out-of-range engine-settings values with `400` + a message; when any engine-settings section is present, all running instances are **recharged live and CONCURRENTLY** after the save (v11.12.3); a failed recharge is **surfaced in the response body** (`Configuration successfully saved. Warning: pipeline recharge failed …`) instead of a silent log. The v11.9 `timeframes` save rides the same recharge path — running instances rebuild their pipelines to the new ACTIVE duration set. `500 Internal Server Error` if `config.toml` cannot be written. |
...
- `clusters`, `volume_profiles`, and `liquidity_flows` are per-timeframe-slot maps keyed by the fixed slot names (`1s`/`3s`/`5s`/`15s`/`30s`/`1m`/`3m`/`5m`/`15m`/`1h`); each is omitted when empty.
...
| `POST` | `/api/instances/:instance_id/reload` | **Served (v7.1):** tear down + rebuild a single TF pipeline (`?tf=<secs|label|all>`, e.g. `tf=60` / `tf=1m` / `tf=all`) or every ACTIVE duration (`tf=all` — a full recharge rebuilds exactly the ACTIVE pipelines). See [08-08 CB-11](../operations-and-compliance/08-08-candle-buffer-spec.md) and [03-01-06 DCP-09](../engines/data-infrastructure-engine/03-01-06-die-candle-pipeline-states.md). |
...
| `GET` | `/api/instances/:instance_id/automation` | **v7 TAE surface (served).** Full setup-executor state: mode (observe/paper/live; the payload's `ghost` flag marks observe), phase, tracked setup + projected risk/return, entry + bracket orders, position, invalidation state, activity log, safety gate, lifecycle, equity. See [03-03-01 §8.1](../engines/trade-automation-engine/03-03-01-tae-overview-spec.md). |
...
| `GET` | `/api/liquidity/cluster-status?symbol=&slot=` | Per-TF `LiquidationClusterMatrix` refresh status (`ClusterStatusSnapshot`: `{ symbol, timeframe_label, status, reason?, generated_at_ms?, valid_until_ms?, cluster? }` with status `Pending` while the refresh task has not produced a matrix, `Skipped` + reason when the per-TF kill switch or `[activation] cluster_estimation` disables it, `Ready` otherwise). Response shape adapts to the query: `?symbol=&slot=` → flat single snapshot; `?symbol=` → `{ symbol, slots: { 1s|3s|5s|15s|30s|1m|3m|5m|15m|1h: snapshot } }`; no filter → `[{ symbol, slots }]`. |
...
### 2.12 Account & strategy endpoints (served)

| Method | Endpoint | Purpose |
|--------|----------|---------|
| `GET` | `/api/account/summary` | **Served (v9).** Aggregated account state: `portfolio_capital_source` (`paper_config \| exchange \| none`), value, equity, realized/unrealized, daily PnL, drawdown, peak, margin usage, safety state, instance + open-position counts. |
| `POST` | `/api/account/capital` | **Served (v9) (paper only; 400 in observe/live).** `{ portfolio_capital_usd }` → session default for new sessions; existing ledgers never silently reseeded. |
| `POST` | `/api/account/reset` | **Served (v9) (paper only).** Reseed the ledger to the configured capital (audit event). |
| `GET` | `/api/strategies` | **Served (v9).** List strategies (name, base, description, schema_version). |
| `POST` | `/api/strategies` | **Served (v9).** Create/update a strategy JSON (schema_version + M8 validation + coherence warnings). |
| `PUT` | `/api/strategies/:name` | **Served (v9).** Update a strategy JSON. |
| `DELETE` | `/api/strategies/:name` | **Served (v9).** Delete a strategy (blocked when bound to instances; `default` is locked). |
| `POST` | `/api/strategies/:name/clone` | **Served (v9).** Clone a strategy under a new name. |
| `POST` | `/api/instances/:id/lifecycle` | **Served (v9+; the canonical lifecycle control).** `{ action: "start" \| "pause" \| "terminate" }` — start → RUNNING; pause → **instance PAUSED** (close-only: no new entries, pending orders cancelled, open positions managed normally); terminate → STOPPED (cancel all orders, force-close at market, `exit_reason = "terminated"`). |

### 2.13 Performance Analytics endpoints (live)

The Performance Analytics Engine exposes serving endpoints under `/api/analytics/*` and `/api/backtest/*`. These are **live** and consumed by the `PerformanceDashboard` Overview / Strategy / Risk Metrics / Regime Map / Trade Analytics panels (the Backtesting surface moved to BTE in v8).
...
| `POST` | `/api/backtest/run` | Start a backtest — **v8.2 async**: returns `{ run_id, status: "running" }` immediately. Payload, two forms: standalone `{ exchange, symbols: [{ symbol, timeframes: [60, 180, 300, 900, 3600], allocation_pct }], initial_capital, from_ms, to_ms, mode? }` (no running instance required; `timeframes` = 1..=14 strictly-ascending archive-eligible values ≥ 60 s — the sub-minute durations are live-only — default `[60, 180, 300, 900, 1800, 3600, 14400, 43200, 86400]`; Σ allocations ≤ 100 % enforced; Hyperliquid depth validated against the 5,000-candle ceiling per TF) or bound `{ symbol, timeframe_secs, from_ms, to_ms, initial_capital, instance_id?, mode? }` (v8 backward compat; **v11.2** — a bound run replays the bound instance's ACTIVE ladder ∩ ≥ 60 s, i.e. `active_secs` filtered to the archive floor; if that set is empty — e.g. a sub-minute-only ladder — the run is rejected `400 no_active_ladder` with a "raise `[workspace].timeframes` past the 60 s durations to backtest" message, and a `timeframe_secs` outside the resolved set is rejected `400` naming the set). `mode` is `recorded` (replays recorded MME decisions through the setup executor, paper only) or `historical` (full MME pipeline over the candle archive, multi-symbol). ms→seconds conversion server-side; `400 not_enough_data` with coverage numbers; `400` ceiling/Σ violations naming the limiting TF; global run lock → `409 backtest_busy`. See [BTE overview](../engines/backtesting-engine/08-01-bte-overview.md). The significance treatment (α, Monte Carlo runs, min trades) comes from `[workspace.analytics]`. |
...
Every notification carries `timeframe_secs` (the authoritative identity) and `timeframe_label` (the derived display label). New connections select their duration explicitly via `?tf=<secs>` (e.g. `?tf=60`) or `?slot=<label>` (e.g. `?slot=1m`); when both are absent the server falls back to the 60 s duration. Only ACTIVE durations emit frames (v11.9 — `[workspace].timeframes` governs which durations have pipelines).
...
- The frontend maintains **N parallel connections per instance** — one per ACTIVE duration (N = `[workspace].timeframes`, 1..=14, default 8; v11.9 — inactive durations open no socket and emit no frames).
