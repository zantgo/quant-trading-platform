...
| `POST` | `/api/instances/:instance_id/reload` | **Served (v7.1):** tear down + rebuild a single TF pipeline (`slot=1s|3s|5s|15s|30s|1m|3m|5m|15m|1h`) or every ACTIVE slot (`slot=all` — a full recharge rebuilds exactly the fastest-N ACTIVE pipelines, v11.2). See [08-08 CB-11](../operations-and-compliance/08-08-candle-buffer-spec.md) and [03-01-06 DCP-09](../engines/data-infrastructure-engine/03-01-06-die-candle-pipeline-states.md). |
