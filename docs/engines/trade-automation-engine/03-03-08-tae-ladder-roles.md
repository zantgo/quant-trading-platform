...
|------|-----------------------------|---------------|
| `decision_tf` | `1h` (3600 s — slowest) | L3 `bias`/`regime`/`market_quality`, L5 `overall_risk`/`market_stance`, `confidence_assessment` |
| `entry_tf` | `1s` (1 s — fastest) | L4 opportunity zones + entry timing |
...
- Backtest `7d × BTC/ETH/SOL @ 60/180/300/900/3600 s` (the default standalone ladder — `1m`/`3m`/`5m`/`15m`/`1h`) with new defaults: **≥15 trades**, T1-style entries survive to TP, `symbol:""` bug fixed.
