...
*   **Processing:** Calculate alignment indices between the ACTIVE fixed ladder timeframes (the fastest `[workspace].active_timeframes` slots of the `1s`…`1h` pool, v11.2) for trend direction, momentum vectors, and key support/resistance blocks.
...
3.  **Consensus:** The *Alignment Layer* checks for trend agreement across the ACTIVE fixed ladder time horizons (the fastest `[workspace].active_timeframes` slots of the `1s`…`1h` pool, v11.2), updating the **Alignment Matrix**.
