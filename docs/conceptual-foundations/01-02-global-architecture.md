...
*   **Processing:** Calculate alignment indices between the ACTIVE fixed ladder timeframes (the ACTIVE durations (`[workspace].timeframes`, any subset of the `1s`…`1d` pool, v11.9)) for trend direction, momentum vectors, and key support/resistance blocks.
...
3.  **Consensus:** The *Alignment Layer* checks for trend agreement across the ACTIVE fixed ladder time horizons (the ACTIVE durations (`[workspace].timeframes`, any subset of the `1s`…`1d` pool, v11.9)), updating the **Alignment Matrix**.
