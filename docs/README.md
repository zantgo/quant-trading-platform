...
│   ├── 01-04-timeframe-model.md                      ← 14-duration timeframe pool, weighting, UTC alignment
...
├── engines/
│   ├── data-infrastructure-engine/                     (03-01 — 9 files)
...
│   └── 03-01-08-die-bitget-v2-derivatives.md      ← Bitget v2 derivatives telemetry (L1.5)
│   ├── market-monitoring-engine/                       (03-02 — 17 files)
...
│   └── 03-02-17-mme-strategy-config.md              ← strategy config surface for the MME
...
│   │   ├── 03-03-07-tae-strategy-settings.md         ← strategy-driven TAE settings (v10.1)
│   │   └── 03-03-08-tae-ladder-roles.md              ← TF-role separation (decision/entry/stop/target)
│   ├── portfolio-management-engine/                  (03-04 — 6 files)
...
│   │   └── 03-04-06-pme-strategy-settings.md
│   └── performance-analytics-engine/                 (03-05 — 7 files)
...
│       └── 03-05-07-pae-strategy-settings.md
...
| **Trade Automation Engine (TAE)** `03-03` | Setup executor, position sizing, order routing, trade lifecycle | 2 | Execution Matrix | Implemented (backend; hidden from the observe-only sidebar — paper/live via config/CLI/API) |
...
| **Backtesting Engine (BTE)** `08-0x` | Candle archive, deep-history MME-pipeline replay, recorded-decision replay, DS persistence | 2 (historical + recorded) | `backtest_*` data-science tables | Implemented (v8.2 production-ready; hidden from the sidebar — direct URL/CLI) |

> **Implementation status (v11.12).** All six engines are implemented: DIE and MME end-to-end across the ACTIVE duration set; TAE as the setup executor on the unified execution engine (Observe is the default posture and the only wizard mode in this build; paper/live backends remain via config/CLI/API) with the v10 lifecycle-hardening dials; PME as a read-only informational portfolio mirror; PAE with live analytics, the strategy optimizer and the significance treatment; BTE with deep-history simulations and the recorded-decision replay, persisted to normalized data-science tables. See [`docs/ROADMAP.md`](ROADMAP.md) §2 for the engine-by-engine reality.
...
1. **Conceptual Foundations (`01-`)** — overall architecture and terminology
   ...
   - `01-04-timeframe-model.md` — 14-duration timeframe pool + §3.1 UTC alignment rules
...
This table is the **single source of implementation truth** — every spec in `docs/` describes the **target system**; this register tracks what is built. **Implementation status is the registered status at v11.12 (2026-09-19) — the current UI build is observe-only (market monitoring); paper/live execution remains a backend/CLI/API capability. For the detailed phased delivery plan, see [`docs/ROADMAP.md`](ROADMAP.md).**
...
| **MME — Market Monitoring** (52 indicators, 14-duration pool with a live-editable ACTIVE set, 12 SignalKinds, Liquidity Intelligence) | Implemented | `03-02-01`…`03-02-17`, `01-05`, `04-02-00`, `05-02-00` |
| **TAE — Trade Automation** (Setup executor + unified execution engine + Paper/Live backends + Lifecycle) | Implemented — backend capability; the observe-only build hides the dashboards (config/CLI/API launch paper/live; the mode is fixed at launch) | `03-03-01`…`03-03-08`, `ROADMAP.md §2.3` |
| **PME — Portfolio Management** (Position + Exposure + Capital + Overview/Safety) | Implemented — informational mirror (safety-state ladder, read-only) | `03-04-01`…`03-04-06`, `ROADMAP.md §2.4` |
| **PAE — Performance Analytics** (Stats compiler + Strategy NHST + Risk analytics + Optimizer) | Implemented — analytics live; the backtest surfaces moved to BTE (v8) | `03-05-01`…`03-05-07`, `ROADMAP.md §2.5` |
...
| Pre-trade risk controls (lifecycle / safety / strategy intake gates) | Implemented | `08-02` |
| Allocation sizing (`equity × allocation_pct`) | Implemented | `03-03-01`, `03-03-03` |
...
| **BTE backtest runner + equity curve** | Implemented — `POST /api/backtest/run` (async) + `GET /api/backtest/:id`, equity-curve chart; observe sidebar hides BTE (direct URL/CLI) | `08-01` (BTE), `ROADMAP.md §2.5` |
| Instance lifecycle (v10.1: TAE activation IS the lifecycle) | Implemented — `POST /api/instances/:id/lifecycle {start\|pause\|terminate}` + STOP flatten orchestration | `03-03-06` |
...
| Liquidity Intelligence | Implemented — derivatives telemetry, flow, per-duration cluster matrix + price-chart cluster overlay (`LiquidationHeatmapPrimitive`) | `01-05`, `03-02-11` |
...
| **TAE live exchange order dispatch** | Implemented — `LiveBroker` (Hyperliquid, EIP-712) + `BitgetLiveBroker` (Bitget V5 HMAC); observe never dispatches and the mode is fixed at launch (no runtime toggle) | `03-03-03 §5b`, `ROADMAP.md §2.3` |
...
- The **corpus version** is defined by four-point coherence: the value appearing simultaneously in this README's stats line, the `CHANGELOG.md` top entry, the `DOCS-CONSISTENCY-MANIFEST.md` title, and every numbered-doc `**Version:**` stamp (currently 11.12 — see docs/CHANGELOG.md for the canonical version history).
...
- All candle aggregation closes candles at the **exact epoch-duration multiple of UTC** (a 60 s candle for a trade at 123456 ms aligns to `[120000, 180000)`, closing at 180000 ms = `:00.000` of the next minute) — see `01-04-timeframe-model.md §3.1`. Local clock drift budget defaults to 10 ms of UTC (`[clock_monitor] threshold_micros`), enforced at runtime by `crates/network-adapters/src/clock_monitor.rs`.
- Position sizing is **allocation-based**: `notional = equity × allocation_pct/100` (per-instance override; Σ over instances ≤ 100 %) — see `03-03-01-tae-overview-spec.md` and `03-03-03-tae-layer2-execution.md`.
...
- PME is an **informational mirror** — it never blocks entries. The optional strategy intake gates (`pme.exposure.enforce.*`, `pme.enforce_systemic_veto`, default OFF) can refuse new entries from the TAE side; the safety ladder (`DRAWDOWN_STOP` / `SUSPENDED`) is the always-on soft gate. Per-order `reduce_only` remains an exchange-parity boolean on order packets (`03-03-03-tae-layer2-execution.md §3.3`).
